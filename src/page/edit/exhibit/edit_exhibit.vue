  ...
  <div class="add-exhibit-wrapper">
    ...
    <el-form :model="ruleForm" :rules="r_rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      ...
      <el-form-item label="从属类别" :style="{display:'inline-block'}" prop="relicType">
        <el-select v-model="currentType" placeholder="藏品类别">
      ...
      <el-upload class="uploader-box" :show-file-list="false"
                 ...
                 :on-success="uploadCoverSuc">
        <img v-if="imageUrl" :src="imageUrl" class="avatar"><i v-else class="avatar-uploader-icon el-icon-plus"></i>
      </el-upload>
      ...
      <el-upload class="uploader-box" name="uploadFile" list-type="picture-card"
                 action="/museum-api/file/management"
                 :on-success="handleUploadImgs"
                 :data=uploadPrams
                 :on-preview="handlePictureCardPreview"
                 :file-list="tempImageList"
                 :limit=4 :on-exceed="overLimit"
  ...
  export default {
    created(){
      if (this.$route.query.id) {
        // 获取藏品
        this.$api.api_req('museum-api/relic/resources', 'POST', { pageNum: 1, pageSize: 5, relicId: this.$route.query.id }, this.getRelicSuc, this.failure)
        // 获取藏品类别
    ...
    methods: {
      ...
      // 获取藏品成功时的回调
      getRelicSuc(_data){
        this.ruleForm = _data.data[0]
        this.currentType = this.ruleForm.relicTypeStr       // 当前种类 会在watch中使用一个中间量记录状态
        this.imageUrl = this.$HOST + this.ruleForm.picUrl   // 封面 (ele组件本地图片的显示)
        this.coverImgId = this.ruleForm.images[0].id        // 封面id
        this.tempImageList = this.ruleForm.images.slice(1)  // 去掉第一张图片 (封面)
        this.tempImageList.map(item => {                    // url处理
          item.url = this.$HOST + item.url
        })
      },
      // 上传封面
      uploadCoverSuc(res, file) {
        ...
        if (this.coverImgId !== 0) {
          ...
          this.coverImgId = 0 // 初始化coverflag
        }
        this.coverImgId = res.data.id
        this.$api.api_req('museum-api/relic/management/image', 'POST', {relicId: this.ruleForm.id, imageIds: [].concat(res.data.id)}, () => { this.$message({message: '封面上传成功'}) }, this.failure) // 绑定
        this.ruleForm.picUrl = res.data.url
      },
      // 上传图片相册
      handleUploadImgs(res, file){
        this.tempImageList.push({id: res.data.id, tempId: file.uid, url: file.url})
        this.$api.api_req('museum-api/relic/management/image', 'POST',
          {relicId: this.ruleForm.id, imageIds: _.map(this.tempImageList, 'id')},
          this.handleUploadImgsSuc,
          this.failure)
      },
      handleUploadImgsSuc(_data){
        this.$message.success({message: '图片上传成功'})
      },
      // 删除相册
      handleRemove(_data){
        this.$api.api_req(
          'museum-api/relic/management/image', 'DELETE',
          {relicId: this.ruleForm.id, imageId: _data.id},
          () => { this.$message({message: '图片删除成功'}) },
          this.failure
        )
        this.tempImageList = _.reject(this.tempImageList, ['id', _data.id])
      },
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$api.api_req('museum-api/relic/management', 'PUT', this.ruleForm, this.submitFormSuc, this.failure)
          } else {
      ...
      overLimit(){
        this.$message({message: '最多只能上传4个图片'})
      },
      resetForm(formName) {
